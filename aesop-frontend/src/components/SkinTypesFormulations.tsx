export default function SkinTypesFormulations() {
  ...
  return (
    <div className="w-full bg-[#F4F2E9] py-8 md:py-16 lg:py-32 px-4 md:px-6 min-h-[400px] md:min-h-[500px] lg:min-h-[600px]">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 md:mb-16 lg:mb-24">
          ...
          <br className="hidden lg:block" />
          <br className="hidden lg:block" />
          <h1 className="text-2xl md:text-3xl lg:text-3xl font-arial text-[#1C1C1C] leading-snug tracking-tight max-w-4xl relative lg:absolute lg:left-20">
        ...
        {/* Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 md:gap-x-12 lg:gap-x-25 gap-y-8 md:gap-y-16 lg:gap-y-30 min-h-[300px] md:min-h-[400px] lg:min-h-[600px] xl:min-h-[300px] relative lg:absolute lg:left-20 lg:right-20">
          {skinTypes.map((skinType, index) => (
            <div key={index} className="space-y-4 md:space-y-5 lg:space-y-6">
              {/* Title with underline on hover and arrow */}
              <div className="flex items-center justify-between border-t border-[#1C1C1C] pt-4 md:pt-5 lg:pt-6">
                <h3 className="text-base md:text-[17px] lg:text-[18px] font-semibold text-[#1C1C1C] hover:underline cursor-pointer">
                  {skinType.title}
                </h3>
                <svg
                  className="w-4 h-4 md:w-5 md:h-5 text-[#1C1C1C] flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {/* Description */}
              <p className="text-[#4B4B4B] text-sm md:text-[15px] lg:text-[16px] leading-relaxed md:leading-loose">
